...
from crawl_amazon_proudcts_selenium import ACrawler
...
class AcrawlerUI(QDialog):
    def __init__(self):
        super().__init__()
        self.save_file_init_dir_path = current_path
        self.is_accepted = False
        self.ac = ACrawler(self)  
        self.crawler_thread = threading.Thread(target = self.ac.run)
        self.time_worker = TimeDisplayWorker(self)
        self.init_gui()

  
    def init_gui(self):
        self.setWindowIcon(QIcon(os.path.join(current_path, '../img/amazon.png')))
        self.setWindowTitle('아마존 상품 다운로드')
        ...
        self.search_line_edit = QLineEdit("예: 나이키 신발")
        ...
        # 최대 개수와 관련된 위젯 정렬
        self.max_num_page_h_layout = QHBoxLayout()
        self.max_num_page_label = QLabel("최대 페이지 수: ")
        self.max_num_page_line_edit = QLineEdit("[참고] 한 페이지당 대략 50여개의 상품이 있습니다.")
        # self.max_num_page_line_edit.setFixedSize(90, 20)
        
        self.max_num_page_h_layout.addWidget(self.max_num_page_label)
        self.max_num_page_h_layout.addWidget(self.max_num_page_line_edit)
        self.max_num_page_h_layout.setAlignment(Qt.AlignLeft)
        ...
        self.main_v_layout.addLayout(self.max_num_page_h_layout)
    ...
    def accept(self):  
        if self.search_line_edit.text().strip() == '':
                QMessageBox.warning(self, "경고 메세지", "찾을 제품 검색어를 입력해주세요!")
                return
        if self.max_num_page_line_edit.text().strip() == '':
            QMessageBox.warning(self, "경고 메세지", "다운로드 받을 제품 개수를 입력해주세요!")
            return
        if self.save_file_line_edit.text().strip() == '':
            QMessageBox.warning(self, "경고 메세지", "다운받을 디렉토리 경로를 설정해주세요!")
            return
        if re.findall(r'\d+', self.max_num_page_line_edit.text().strip()) == []:
            QMessageBox.warning(self, "경고 메세지", "다운받을 제품 개수에 정수값을 입력해주세요!")
            return  
